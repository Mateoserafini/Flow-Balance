...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Registro (mapeamos el nombre a username)
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Error al intentar registrarse o iniciar sesión.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-ui-bg dark:bg-slate-950 flex items-center justify-center p-4 transition-colors duration-300">
      <div className="bg-white dark:bg-slate-900 shadow-xl rounded-2xl p-8 w-full max-w-md border border-gray-100 dark:border-slate-800">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-ui-text-main dark:text-slate-100 mb-2">Crear Cuenta</h1>
          <p className="text-ui-text-sub dark:text-slate-400">Únete a Flow-Balance para gestionar tus finanzas</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-ui-text-main dark:text-slate-300 mb-1" htmlFor="name">
